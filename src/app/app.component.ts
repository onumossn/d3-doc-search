...
export class AppComponent {
  ...
  githubRequestErrorHandler(err: any) {
    if (_.parseInt(err.headers.get('X-RateLimit-Remaining')) === 0) {
      this.errorMessage = `Github API rate limit exceeded for 76.100.64.34. 
  ...
  constructor(public http: Http) {
    ...
    if (!this.docModel || resetTime < Date.now()) {
      ...
      http.get('https://api.github.com/repos/d3/d3/contents/API.md?ref=master', { headers: headers })
        .subscribe(indexResp => {
          ...
          this.docModel.forEach((indexItem: any, j: number) => {
            let url: string = indexItem.link.split('/');
            url = `https://api.github.com/repos/d3/${url[url.length - 1].split('#')[0]}/contents/README.md?ref=master`;
